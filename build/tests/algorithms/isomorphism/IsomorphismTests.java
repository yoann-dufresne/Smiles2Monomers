...
public class IsomorphismTests {
	...
	public void setUp() throws Exception {
		...
		// Mapped blocs
		this.mb0 = new MappedBloc(pepTest, null, new ArrayList<Integer>(), new ArrayList<Integer>(), new ArrayList<MatchingType>(), new HashMap<Integer, Integer>());
		
		// For blocs Tests
		this.bloc = new Bloc("S,0,c,0,0,-1,-1;c,0,c,0,0,-1,1");
	}
	...
	public void matchingAromaticTest () {
		...
		List<MappedBloc> mbs = Isomorphism.searchABloc(this.bloc, pep, MatchingType.STRONG);
	...
	public void lightMatchingAromaticTest () {
		Polymer pep = new Polymer(0, "", "SC1CCCCC1", null);
		List<MappedBloc> mbs = Isomorphism.searchABloc(this.bloc, pep, MatchingType.LIGHT);
		Assert.assertTrue(mbs.size() > 0);
	}
	
	@Test
	public void initMappingTest() {
		List<MappedBloc> mbs = Isomorphism.searchFromPreviousMapping(this.mb0, this.ext1, MatchingType.STRONG);
		if (mbs.size() != 2)
	...
	public void MappingTest () {
		...
		boolean isGood = true;
		for (MappedBloc mb : mbs) {
			List<MappedBloc> extendedMbs = Isomorphism.searchFromPreviousMapping(mb, this.ext2, MatchingType.STRONG);
			MappedBloc newMb = extendedMbs.get(0);
