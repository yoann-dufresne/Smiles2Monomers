...
public class Bloc implements Comparable<Bloc> {
	 ...
	 */
	public Bloc(Bloc subBloc, Extension ext, int position1, int position2) {
		this.subBlc = subBloc;
		this.ext = ext;
	...
	public Bloc (String serial) {
		String extS = serial;
		int maxSubIdx = -1;
		if (serial.contains(";")) {
			int n = extS.lastIndexOf(";");
			this.subBlc = new Bloc(extS.substring(0, n));
	...
	public int getPerformance(BlocsDB blocs) {
		if (this.getSize() == 1)
			return 1;
		
		Bloc sub = this.getSubBlc();
		int frq = blocs.getFrequency(sub);
		int extensionTime = sub.getSize() * frq;
		int subPerf = sub.getPerformance(blocs);
		
		if (frq == 0 || subPerf == 0)
			return 0;
		else
			return subPerf + extensionTime;
	}
