...
public class DeepenMatcher {
	 ...
	 */
	public static IMolecule mask(Coverage coverage) {
		IMolecule covMol = coverage.getChemicalObject().getMolecule();
		...
		for (IAtom atom : mol.atoms())
			for (IBond bond : covMol.getConnectedBondsList(atom))
				if (mol.contains(bond.getAtom(0)) &&
						mol.contains(bond.getAtom(1)))
					mol.addBond(bond);
